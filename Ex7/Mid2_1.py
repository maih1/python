...
def coshTaylor(x, e):
    ...
    while (check > e):
        cosh += pow(x, i) / factorial(i)        
        check = round(pow(x, i) / factorial(i), 5)
        
        cosh_i = cosh
        i += 2

        cosh_i += pow(x, i) / factorial(i)
        print(cosh, cosh_i, check)
...
def testDemo():
    ...
    # print(round(coshTaylor(5.5, 0.00001), 5))
...
'''
#testDemo()
filename = './Ex7/text/test1.txt'
print(getTop2gram(filename, 5))
print(getVector(filename, (getTop2gram(filename, 5))))
print(round(getDistance([1, 2, 3, 4], [1, 2, 1, 1]), 5))
