...
class TFIDF:
    ...
    def loadCorpus(self,corpusPath):
        ...
        listPath = os.listdir(corpusPath)

        for i in listPath:
            path = corpusPath + '/' + i
            doc = Doc(path)
            self.data.update({i: doc})
    ...
    def getKTopicWordFromCopus(self, k):
        ...
        # print(sets)
        tm = list(sets)
        tm.sort(key=compers, reverse=True)
...
corpusPath = './Ex7/text/data'
# test = Doc(filename)
test2 = TFIDF(corpusPath)

# print(test2.tf('A', test))
# print(test.wordCount)

# print(test2.data)

print(test2.getKTopicWordFromCopus(7))